...
/** 1回の復習セッションで出題する最大単語数 */
export const REVIEW_WORDS_PER_SESSION = 10;
...
export default function ReviewQuizScreen() {
  ...
  const wordIds = useMemo(() => db.getWordsDueForReview(REVIEW_WORDS_PER_SESSION), []);
  ...
  const currentWord = words.length > 0 && order.length > 0 ? words[order[currentIndex]] : null;
  ...
  useEffect(() => {
    if (wordIds.length === 0 && words.length === 0) {
      refresh();
      router.replace('/(tabs)/vocabulary');
    }
  }, [wordIds.length, words.length, router, refresh]);
  ...
  const handleSelect = (answer: string) => {
    ...
    const next = calculateNextReview(card, isCorrect ? 5 : 0);
    ...
    setTimeout(() => {
      if (currentIndex + 1 >= words.length) {
        ...
        db.saveLessonQuizResult('review', resultArray);
        db.setLastQuizWordIds('review', words.map((w) => w.id));
        refresh();
        router.replace({
  ...
  if (wordIds.length === 0 && words.length === 0) {
    return (
      <EmptyStateBlock
        ...
        subtitle={t.reviewEmptySub}
        buttonLabel={t.reviewEmptyButton}
        onPress={() => { void triggerLightImpact(); router.replace('/(tabs)/vocabulary'); }}
        accessibilityLabel={t.reviewEmptyButtonA11y}
      />
    );
  }
  ...
  return (
    <ErrorBoundary contextLabel={nav.quizReview}>
    <View style={styles.container}>
      <View
        style={[styles.header, { paddingTop: headerPaddingTop }]}
        accessibilityRole="header"
        accessibilityLabel={t.quizHeaderA11y(nav.quizReview, currentIndex + 1, words.length)}
      >
        <View style={styles.headerRow}>
          ...
          <Text style={styles.headerTitle}>{nav.quizReview}</Text>
